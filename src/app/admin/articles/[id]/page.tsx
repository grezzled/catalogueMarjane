    ...
    if (line.trim().startsWith("|") && line.trim().endsWith("|")) {
      ...
      if (inTableBlock && tableRows.length > 0) {
        ...
        const headerCells = headerRow
          .split("|")
          .filter((c) => c.trim())
          .map((c) => `<th class="px-4 py-3 bg-gray-50 border border-gray-200 text-left font-bold text-gray-900 text-sm">${c.trim()}</th>`)
          .join("");
        const rowsHtml = dataRows
          .map((row) => {
            const cells = row
              .split("|")
              .filter((c) => c.trim())
              .map((c) => `<td class="px-4 py-3 border border-gray-200 text-sm">${c.trim()}</td>`)
              .join("");
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              ...
              <div className="flex flex-wrap gap-3">
                {article.catalogue?.id && (
                  <button
                    ...
                    disabled={rewriting || actionLoading}
          ...
          <div className="space-y-6">
            ...
            {article.catalogue && (
              <div className="bg-white rounded-lg shadow p-6">
                <h2 className="text-lg font-bold text-gray-900 mb-3">Catalogue</h2>
